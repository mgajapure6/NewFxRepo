...
public final class JavaBeanResult implements QueryRedirector {
	...
	public static void setQueryResultClass(DatabaseQuery query, Class<?> resultClass) {
		JavaBeanResult javaBeanResult = new JavaBeanResult(resultClass);
	...
	public Object invokeQuery(DatabaseQuery query, Record arguments, Session session) {
		...
		try {
			Constructor<?> javaBeanClassConstructor = (Constructor<?>) resultClass.getDeclaredConstructors()[0];
			Class<?>[] constructorParameterTypes = javaBeanClassConstructor.getParameterTypes();
			...
			for (Object[] columns : rows) {
				...
				if (columns.length != constructorParameterTypes.length) {
					throw new ColumnParameterNumberMismatchException(resultClass);
				}
				for (int j = 0; j < columns.length; j++) {
					...
					constructorArgs[j] = ConversionManager.getDefaultManager().convertObject(columnValue,
							parameterType);
				}
	...
	public final class ColumnParameterMismatchException extends RuntimeException {
		...
		public ColumnParameterMismatchException(Throwable t) {
			super("Exception while processing query results-ensure column order matches constructor parameter order",
					t);
		}
	}
	...
	public final class ColumnParameterNumberMismatchException extends RuntimeException {
		...
		public ColumnParameterNumberMismatchException(Class<?> clazz) {
			super("Number of selected columns does not match number of constructor arguments for: " + clazz.getName());
		}
	}
}
